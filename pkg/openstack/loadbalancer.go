...
func (lbaas *LbaasV2) ensureOctaviaListener(lbID string, name string, curListenerMapping map[listenerKey]*listeners.Listener, port corev1.ServicePort, svcConf *serviceConfig, service *corev1.Service) (*listeners.Listener, error) {
	...
	if !isPresent {
		...
		if svcConf.supportLBTags {
			...
			copy(originalTags, listener.Tags)

			var tagsChanges []string
			copy(tagsChanges, listener.Tags)

			if !cpoutil.Contains(listener.Tags, svcConf.lbName) {
				tagsChanges = append(tagsChanges, svcConf.lbName)
			}

			expectedTags := getCustomLoadBalancerTags(service, svcConf)
			// find missing tags from the annotation for custom tags
			for _, expectedTag := range expectedTags {
				if !cpoutil.Contains(listener.Tags, expectedTag) {
					tagsChanges = append(tagsChanges, expectedTag)
				}
			}
			// Find tags which aren't presented in the annotation
			for _, presentedTag := range originalTags {
				// we don't want to remove tags which contain kube_service in them, as they are generated by CPO.
				if strings.Contains(presentedTag, "kube_service") {
					continue
				}
				if cpoutil.Contains(expectedTags, presentedTag) {
					// removes the element from tagsChanges slice
					s := slices.Index(tagsChanges, presentedTag)
					tagsChanges = append(tagsChanges[:s], tagsChanges[s+1:]...)
				}
			}
			// basically check if tagsChanges == originalTags, if NOT then setup newTags and other vars
			if len(tagsChanges) != len(originalTags) {
				var newTags []string
				copy(newTags, tagsChanges)
				updateOpts.Tags = &newTags
				listenerChanged = true
			} else { // lens are equal check if tags are changed
				for _, tag := range originalTags {
					if !cpoutil.Contains(tagsChanges, tag) {
						var newTags []string
						copy(newTags, tagsChanges)
						updateOpts.Tags = &newTags
						listenerChanged = true
...
func (lbaas *LbaasV2) ensureOctaviaLoadBalancer(ctx context.Context, clusterName string, service *corev1.Service, nodes []*corev1.Node) (lbs *corev1.LoadBalancerStatus, err error) {
	...
	if !createNewLB && isLBOwner {
		var originalLBTags []string
		copy(originalLBTags, loadbalancer.Tags)

		var tagsChanges []string
		copy(tagsChanges, loadbalancer.Tags)

		expectedTags := getCustomLoadBalancerTags(service, svcConf)
		for _, expectedTag := range expectedTags {
			if !cpoutil.Contains(originalLBTags, expectedTag) {
				tagsChanges = append(originalLBTags, expectedTag)
			}
		}

		for _, presentedTag := range originalLBTags {
			if !cpoutil.Contains(expectedTags, presentedTag) {
				// we don't want to remove tags which contain kube_service in them, as they are generated by CPO.
				if strings.Contains(presentedTag, "kube_service") {
					continue
		...
		if len(tagsChanges) != len(originalLBTags) {
			klog.InfoS("Updating load balancer tags", "lbID", loadbalancer.ID, "tags", tagsChanges)
			if err := openstackutil.UpdateLoadBalancerTags(lbaas.lb, loadbalancer.ID, tagsChanges); err != nil {
				return nil, err
			}
		} else { // lens are equal check if tags are changed
			for _, tag := range originalLBTags {
				if !cpoutil.Contains(tagsChanges, tag) {
					klog.InfoS("Updating load balancer tags", "lbID", loadbalancer.ID, "tags", tagsChanges)
					if err := openstackutil.UpdateLoadBalancerTags(lbaas.lb, loadbalancer.ID, tagsChanges); err != nil {
						return nil, err
