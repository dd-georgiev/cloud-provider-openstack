...
// Source: https://stackoverflow.com/a/36000696
...
func (lbaas *LbaasV2) createOctaviaLoadBalancer(name, clusterName string, service *corev1.Service, nodes []*corev1.Node, svcConf *serviceConfig) (*loadbalancers.LoadBalancer, error) {
	...
	if !lbaas.opts.ProviderRequiresSerialAPICalls {
		for portIndex, port := range service.Spec.Ports {
			// Since load balancer is being created, we know that this service will be the owner(last argument of buildListenerCreateOpt func)
			listenerCreateOpt := lbaas.buildListenerCreateOpt(port, svcConf, service, cpoutil.Sprintf255(listenerFormat, portIndex, name), true)
...
func (lbaas *LbaasV2) ensureOctaviaListener(lbID string, name string, curListenerMapping map[listenerKey]*listeners.Listener, port corev1.ServicePort, svcConf *serviceConfig, service *corev1.Service, isLBOwner bool, loadbalancerTags []string) (*listeners.Listener, error) {
	listener, isPresent := curListenerMapping[listenerKey{
		Protocol: getListenerProtocol(port.Protocol, svcConf),
		Port:     int(port.Port),
	}]
	if !isPresent {
		listenerCreateOpt := lbaas.buildListenerCreateOpt(port, svcConf, service, name, isLBOwner)
		...
		if svcConf.supportLBTags {
			var currentListenerTags []string
			copy(currentListenerTags, listener.Tags)
			var expectedTags []string
			if isLBOwner {
				expectedTags = getCustomLoadBalancerTags(service, svcConf)
			} else {
				// Remove tag from the original kube service
				for i, tag := range loadbalancerTags {
					if strings.Contains(tag, "kube_service") {
						loadbalancerTags = append(loadbalancerTags[:i], loadbalancerTags[i+1:]...)
					}
				}
				expectedTags = append(expectedTags, loadbalancerTags...)
			}
			expectedTags = append(expectedTags, svcConf.lbName)
			if len(currentListenerTags) != len(expectedTags) || !sameStringSlice(currentListenerTags, expectedTags) {
				klog.InfoS("Listener tags need update", "listenerID", listener.ID, "lbID", lbID, "current tags", currentListenerTags, "new tags", expectedTags)
				updateOpts.Tags = &expectedTags
...
func (lbaas *LbaasV2) buildListenerCreateOpt(port corev1.ServicePort, svcConf *serviceConfig, service *corev1.Service, name string, isLBOwner bool) listeners.CreateOpts {
...
func (lbaas *LbaasV2) ensureOctaviaLoadBalancer(ctx context.Context, clusterName string, service *corev1.Service, nodes []*corev1.Node) (lbs *corev1.LoadBalancerStatus, err error) {
	...
	// The load balancer is created and the current service is the owner of it. In such case, its time to check if the tags on the LB
	// Match the tags in the annotation.
	if !createNewLB && isLBOwner {
		var currentLBTags []string
		copy(currentLBTags, loadbalancer.Tags)

		expectedTags := getCustomLoadBalancerTags(service, svcConf)
		expectedTags = append(expectedTags, lbName)
		if len(currentLBTags) != len(expectedTags) || !sameStringSlice(expectedTags, currentLBTags) {
			klog.InfoS("Updating LB tags", "lbID", loadbalancer.ID, "current tags", currentLBTags, "change to ", expectedTags)
			if err := openstackutil.UpdateLoadBalancerTags(lbaas.lb, loadbalancer.ID, expectedTags); err != nil {
				return nil, err
			}
		}
	...
	if !createNewLB || (lbaas.opts.ProviderRequiresSerialAPICalls && createNewLB) {
		...
		for portIndex, port := range service.Spec.Ports {
			listener, err := lbaas.ensureOctaviaListener(loadbalancer.ID, cpoutil.Sprintf255(listenerFormat, portIndex, lbName), curListenerMapping, port, svcConf, service, isLBOwner, loadbalancer.Tags)
			if err != nil {
				return nil, err
			}
